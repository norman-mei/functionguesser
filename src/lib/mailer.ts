...
function buildFromAddress() {
  ...
  return 'Function Guesser <no-reply@functionguesser.app>';
}

export async function sendVerificationEmail(to: string, token: string, newEmail?: string) {
  if (!BREVO_HOST || !BREVO_USER || !BREVO_PASS) {
    throw new Error('SMTP credentials are missing. Check your environment variables.');
  }
  ...
  const subject = isEmailChange
    ? 'Confirm your new Function Guesser email'
    : 'Verify your Function Guesser account';
  const text = isEmailChange
    ? [
        'You requested to change your Function Guesser sign-in email.',
        '',
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #0f172a;">
      <h2 style="color: #6366f1; margin-bottom: 16px;">One quick step left</h2>
      <p>${
        isEmailChange
          ? 'Tap the button below to confirm your new sign-in email for Function Guesser.'
          : 'Tap the button below to verify your email address and finish setting up your Function Guesser account.'
      }</p>
      ...
      <p style="word-break: break-all;">
        <a href="${verifyUrl.toString()}" style="color: #6366f1;">
          ${verifyUrl.toString()}
        </a>
      </p>
